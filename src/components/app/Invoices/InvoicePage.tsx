...
import InvoiceService, { DEFAULT_GET_INVOICES_PARAMS, Invoice } from '@/API/Resources/v1/Invoice/Invoice.Service.ts';
...
import { mergePathNameAndSearchParams, updateOrAddSearchParam, updateOrAddSearchParams } from '@/util/urlUtil.ts';
...
type OnInvoiceAddOrEditSuccess = (action_type: 'add' | 'edit', invoice_id: number) => void;
...
export default function InvoicePage() {
  ...
  const isDetailsPageOpen: boolean = !!(selectedInvoiceId && selectedInvoiceId > 0);
  ...
  const [isDashboardLoading, setIsDashboardLoading] = React.useState<boolean>(true);
  const [, setIsEditModalOpen] = useState<boolean>(false);
  const [, setEditingItemId] = useState<number>();
  const [appliedFilter, setAppliedFilter] = useState<InvoiceAppliedFilter>(INVOICE_DEFAULT_FILTER_BY);
  const [invoiceDashboardData, setInvoiceDashboardData] = useState<InvoiceDashboardData>({
    currency_symbol: '',
    due_today: 0,
  ...
  const loadInvoices = useCallback((search_query_string: string) => {
    ...
    const filterBy = ValidityUtil.optionalChainNotEmpty<string>([filter_by_query], INVOICE_DEFAULT_FILTER_BY);
    const page = Number(ValidityUtil.optionalChainNotEmpty([page_query], DEFAULT_PAGE_NUMBER));
    const per_page = Number(ValidityUtil.optionalChainNotEmpty([per_page_query], DEFAULT_PAGE_SIZE));
  ...
  const handleAppliedFilterChange = (value: InvoiceAppliedFilter) => {
    navigate(
      mergePathNameAndSearchParams({
        ...
        search_params: updateOrAddSearchParams(search, [
          {
            key: 'page',
            value: '1',
          },
  ...
  return (
    <>
      <div className={'w-full h-full flex '}>
        ...
        >
          <section className={'flex px-5 py-3  justify-between items-center drop-shadow-sm border-r-1'}>
            <div>
              <h1 className={'text-lg'}>Invoices</h1>
              <Select value={appliedFilter} onValueChange={handleAppliedFilterChange}>
            ...
            <div className={'flex gap-x-2'}>
              ...
              <DropdownMenu modal={false}>
                ...
                <DropdownMenuContent align={'end'} className="text-sm bg-gray-50 outline-none  p-1 w-56">
                  <DropdownMenuLabel>Sort by</DropdownMenuLabel>
                  {sortOptions.map((option, index) => (
                    <DropdownMenuItem key={index} role={'button'} onClick={() => console.log(option.value)} className={'menu-item-ok'}>
                      {option.label}
                    </DropdownMenuItem>
                  ))}
                  <DropdownMenuSeparator />
                  <DropdownMenuGroup className={'bg-green-50'}>
                    <DropdownMenuItem role={'button'} className={'menu-item-ok text-green-700'} onClick={handleListRefresh}>
          ...
          <div className={'overflow-y-auto flex-grow border-r-1'}>
            {
              ...
              !isDetailsPageOpen && (
                <div className={' px-3'}>
                  <InvoiceDashBoard invoiceDashboardData={invoiceDashboardData} isLoading={isDashboardLoading} onRefresh={handleDashboardRefresh} />
                </div>
              )
            }
            <InvoiceListing
...
export type { OnInvoiceAddOrEditSuccess, OnInvoiceDeleteSuccess, OnInvoiceModification };
