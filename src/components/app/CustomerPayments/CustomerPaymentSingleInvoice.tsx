...
import CustomerPaymentService from "@/API/Resources/v1/CustomerPayment/CustomerPayment.Service.ts";
...
export default function CustomerPaymentSingleInvoice() {
  ...
  const loadEditPage = useCallback(() => {
    customerPaymentService
      .getEditPageFromInvoice({
        invoice_id: Number(invoice_id_param),
      })
      .then((data) => {
        // todo: set the data to the form
      })
  ...
  return (
    <div className={"flex flex-col h-full relative"}>
              ...
              <div className={"w-11/12 max-w-[1000px] space-y-4"}>
                ...
                <div>
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem
                        className={"space-y-0 flex gap-x-2 items-center w-1/2"}
                      >
                        <FormLabel
                          htmlFor={"contact"}
                          className="capitalize label-required w-1/3"
                        >
                          Customer
                        </FormLabel>
                  ...
                  {showAutoNumberSelection && (
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem
                          className={
                            "space-y-0 gap-x-2 flex items-center w-1/2"
                          }
                        >
                          <FormLabel
                ...
                <div>
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem
                        className={"space-y-0 gap-x-2 flex  items-center w-1/2"}
                      >
                        <FormLabel
                          htmlFor={"payment_number"}
                          className=" capitalize label-required w-1/3"
            ...
            <div className={"mt-5 flex p-5 w-11/12 max-w-[1000px] flex-wrap"}>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem
                    className={
                      "space-y-0 gap-x-2 flex  items-center w-1/2 mb-5"
                    }
                  >
                    <FormLabel
                      htmlFor={"amount"}
      ...
      <div
        className={
          " h-16 mb-12 py-2 px-5 flex items-center space-x-2 border-t-1 z-10 bg-background "
        }
      >
        <Button
